...
defineProps<Database["public"]["Tables"]["realisateur"]["Row"] & { films: Tables<'films'>[] }>()
    ...
    <div class="bg-gradient-to-tr from-blue-900 via-blue-800 to-blue-500">
        <div class="bg-slate-400 lg:mx-72  pt-32">
            ...
            <div class="flex items-center justify-around">
                <img :src="image ?? undefined" class="  object-cover  w-32 h-40 lg:w-48 lg:h-64 "
                    alt="affiche du film" />

                <div class="space-y-4 lg:text-2xl text-sm">
                    <p> <b>Age :</b> : {{ age }}</p>
                    <p> <b>Pays :</b> {{ pays }}</p>
                    <p> <b>Nombres de films réalisé :</b> {{ nb_films }}</p>
                </div>
            </div>
            ...
            <ul class="grid grid-cols-2 lg:grid-cols-3 ">
                <li class="m-4" v-for="unFilm in films">
                    <RouterLink :to="{ name: '/films/[id]', params: { id: unFilm.id } }">
                        <img :src="unFilm?.affiche ?? undefined"
                            class="lg:w-56 lg:h-80 w-36 h-56 object-cover m-auto" />
                    </RouterLink>
                </li>
            </ul>
